...
export const deleteWorkflowById = async (workflowId: number) => {
    try{
        const result = await db.transaction(async (tx) => {
            const executionRows = await tx.select({ id: execution.id })
                .from(execution)
...
export const updateWorkflowSchedule = async (workflowId: number, scheduleEnabled: boolean, scheduleIntervalSeconds: number | null) => {
    try {
...
export const saveWorkflowGraph = async (
    ...
    try {
        const result = await db.transaction(async (tx) => {
            ...
            await tx.delete(triggers).where(
            ...
            await tx.delete(integrations).where(
                integrationNodeIds.length > 0
                    ? and(eq(integrations.workflowId, workflowId), notInArray(integrations.nodeId, integrationNodeIds))
                    : eq(integrations.workflowId, workflowId)
            );

            for (const t of triggerRows) {
                await tx.insert(triggers)
            ...
            for (const i of integrationRows) {
                await tx.insert(integrations)
                    .values({ workflowId, nodeId: i.nodeId, provider: i.provider, credentialsJson: i.credentialsJson })
                    .onConflictDoUpdate({
                        target: [integrations.workflowId, integrations.nodeId],
                        set: { provider: i.provider, credentialsJson: i.credentialsJson, updatedAt: new Date() },
                    });
            }
            ...
            const webhookTriggersNeedingToken = await tx.select({ id: triggers.id })
                .from(triggers)
                .where(and(
                    eq(triggers.workflowId, workflowId),
            ...
            for (const t of webhookTriggersNeedingToken) {
                await tx.update(triggers)
                    .set({ webhookToken: generateWebhookToken(), updatedAt: new Date() })
                    .where(eq(triggers.id, t.id));
            }

            const currentTriggers = await tx.select().from(triggers).where(eq(triggers.workflowId, workflowId));
