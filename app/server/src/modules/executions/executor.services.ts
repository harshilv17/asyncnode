import { getWorkflowById, getTriggersByWorkflowId, getIntegrationsByWorkflowId, updateWorkflowSchedule, getTriggerByWebhookToken } from "../workflows/workflow.repo.ts";
...
export const buildExecutionGraph = async (workflowId: number, userId: number, triggerOverrideData?: Record<string, any>) => {
    try {
        ...
        const triggersByNodeId = new Map<string, typeof triggers.$inferSelect>(triggerRows.map((t) => [t.nodeId, t]));
        const integrationsByNodeId = new Map<string, typeof integrations.$inferSelect>(integrationRows.map((i) => [i.nodeId, i]));
...
export const executeWorkflow = async (
    ...
    try {
        const executionContext = await globalExecutor(
            ...
            graph.edges,
            {
                onNodeStart: async (nodeId, nodeType, input) => {
                    const nodeExecutionRow = await createNodeExecution(executionRow.id, nodeId, nodeType, input);
                ...
                onNodeComplete: async (nodeId, status, output) => {
                    ...
                    if (nodeExecutionId !== undefined) {
                        await completeNodeExecution(nodeExecutionId, status === "success" ? "completed" : "failed", output);
                    }
                },
        ...
        await completeExecution(executionRow.id, executionOutcome === "success" ? "completed" : "failed");
...
export const startWorkflowScheduleService = async (workflowId: number, userId: number, intervalSeconds: number) => {
...
export const resolveWebhookTrigger = async (webhookToken: string) => {
    const trigger = await getTriggerByWebhookToken(webhookToken) as typeof triggers.$inferSelect | undefined;
    ...
    const workflow = await getWorkflowById(trigger.workflowId) as typeof workflows.$inferSelect | undefined;
